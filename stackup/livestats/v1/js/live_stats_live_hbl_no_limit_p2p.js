...
function loadP2P(data) {
    for (var i = 0; i < data.score_home_p2p.length; i++) {
        var temp = {
            score_home_p2p: data.score_home_p2p[i],
            score_away_p2p: data.score_away_p2p[i],
            fouls_home_p2p: data.fouls_home_p2p[i],
            fouls_away_p2p: data.fouls_away_p2p[i],
            timeouts_home_p2p: data.timeouts_home_p2p[i],
            timeouts_away_p2p: data.timeouts_away_p2p[i],
        }
        addP2P(temp, i);
    }
}
...
$(document).on('click', '#js-start-period2', function (e) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (!$('#js-p2p-period' + cur_period).length) {
                var data = {
                    score_home_p2p: 0,
                    score_away_p2p: 0,
                    fouls_home_p2p: 0,
        ...
        if (pass_validation) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (side == 'side=true')
                        $('#js-p2p-timeout-home' + (cur_period-1)).text(parseInt($('#js-p2p-timeout-home' + (cur_period-1)).text()) + 1);
                    else
                        $('#js-p2p-timeout-away' + (cur_period-1)).text(parseInt($('#js-p2p-timeout-away' + (cur_period-1)).text()) + 1);
                },
                error: function (request, status, error) {
                    retry(this, request, error);
                }
...
function updateGameScoreAndBoxScore(game) {
    game.teamstats.forEach(function (team, index) {
        if (team.side) {
            ...
            //Update p2p
            if (team.period_stats.length == cur_period) {
                $('#js-p2p-point-home' + (cur_period-1)).text(team.period_stats[cur_period - 1].points);
                $('#js-p2p-pfoul-home' + (cur_period-1)).text(team.period_stats[cur_period - 1].pfoul);
            }
        } else {
            ...
            //Update p2p
            if (team.period_stats.length == cur_period) {
                $('#js-p2p-point-away' + (cur_period - 1)).text(team.period_stats[cur_period - 1].points);
                $('#js-p2p-pfoul-away' + (cur_period - 1)).text(team.period_stats[cur_period - 1].pfoul);
            }
        }
    });
...
function initXEditable() {
    $('.js-instant-play-category').editable({
        ...
        type: 'select',
        source: [
            {value: STAT_CATEGORY_FT_M, text: '罰球 中'},
            {value: STAT_CATEGORY_FT_A, text: '罰球 不中'},
            {value: STAT_CATEGORY_TWO_M, text: '2分中'},
            {value: STAT_CATEGORY_TWO_A, text: '2分不中'},
            {value: STAT_CATEGORY_TREY_M, text: '3分中'},
            {value: STAT_CATEGORY_TREY_A, text: '3分不中'},
            {value: STAT_CATEGORY_REB_O, text: '進攻籃板'},
            {value: STAT_CATEGORY_REB_D, text: '防守籃板'},
            {value: STAT_CATEGORY_AST, text: '助攻'},
            {value: STAT_CATEGORY_BLK, text: '阻攻'},
            {value: STAT_CATEGORY_STL, text: '抄截'},
            {value: STAT_CATEGORY_TURNOVER, text: '失誤'},
            {value: STAT_CATEGORY_PFOUL, text: '個人犯規'},
        ],
